<%@ Page Language="C#" Inherits="Virweb2.FormList.FormIPPList" Codebehind="FormIPPList.aspx.cs" %>
...
*/
        ...
        function add_click(uci)
        {
            if (document.IPPList.ippexist.value == "1")
            {
               ...
               var saveflag = window.showModalDialog("../webSaveChange.aspx?DspMsg=" + DspStr, self, 
                             "status:no;dialogWidth:350px;dialogHeight:120px;help:no;scroll:no;resizable:yes;");
               if (saveflag == "1")
                   document.location.replace("FormIPPInfo.aspx?uci="+uci+"&type=add&Prefill=1");
               else if (saveflag == "0")
                   document.location.replace("FormIPPInfo.aspx?uci="+uci+"&type=add");
            }
            else
               document.location.replace("FormIPPInfo.aspx?uci="+uci+"&type=add");
        }
