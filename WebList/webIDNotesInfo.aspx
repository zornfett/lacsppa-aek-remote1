<%@ page language="C#" inherits="Virweb2.WebList.webIDNotesInfoSCLARC" ValidateRequest="false" Codebehind="webIDNotesInfo.aspx.cs" AutoEventWireup="True" %>
...
*/
    ...
    <script type="text/javascript">
        ...
        {
           if (Confirm_Page())
              document.location.replace("webIDNotesInfo.aspx?type=add&validated="+document.IDNotesInfo.validated.value);
        }
        ...
        function List_click()
        {
            if (Confirm_Page())
            {
               if (document.getElementById("hdtoSaveData").value=="DeleteDraft")
                  autodelete();

               //document.location.replace('webConsumerList.aspx');
               if (document.IDNotesInfo.groupID.value != "")
                  document.location.replace("webIDNotesList.aspx?validated="+document.IDNotesInfo.validated.value+"&groupID="+document.IDNotesInfo.groupID.value); 
               else
                  document.location.replace("webIDNotesList.aspx?validated="+document.IDNotesInfo.validated.value); 
            }
        }
       ...
       function submitcalendar(boxid) {
        ...
        function cmnotessr_click(){
           popupDialogSmall('googlesr.htm');
        }

        function on_sr_win_cmd(cmd, sr_param,v){        
            if (cmd=="sr_cancel"){
            }   
            if (cmd=="sr_ok"){
               document.getElementById("tx_cm_notes").value = document.getElementById("tx_cm_notes").value +"\r"+v;
            }
                
        }
        /*
        function AutoAddNewIDNotes()
        {
           document.getElementById("hdtoSaveData").value="AutoAdd";
           window.document.IDNotesInfo.submit(); 
           window.focus(); 
        }
        */
        
        function PopupDraftInfo()
        {
           if (confirm('There is a draft IDNote saved in previous session, do you want to restore it ?'))
             document.getElementById("hdtoSaveData").value="FillDraft";
           else
             document.getElementById("hdtoSaveData").value="DeleteDraft";
        ...
        function autosave() {
            ...
            $.ajax({
              type: "POST",
              data: {
                     consumer_key:$("#<%=consumer_key.ClientID %>").val(),
                     txdate:$("#<%=txdate.ClientID %>").val(),
                     tx_date:$("#<%=tx_date.ClientID %>").val(),
                     l_tx_date_min:$("#<%=l_tx_date_min.ClientID %>").val(),
                     tx_type:$("#<%=tx_type.ClientID %>").val(),
                     tx_template:$("#<%=tx_template.ClientID %>").val(),
                     tx_minutes:$("#<%=tx_minutes.ClientID %>").val(),
                     tx_rid:$("#<%=tx_rid.ClientID %>").val(),
                     tx_id_notes:$("#<%=tx_id_notes.ClientID %>").val(),
                     tx_cm_notes:$("#<%=tx_cm_notes.ClientID %>").val()
                    },  
              //contentType: "application/json; charset=utf-8",
              url: 'webIDNotesInfo.aspx?isAjax=true&autosave=1',
              dataType: "json",
        ...
        function autodelete() {
            $.ajax({
              ...
              //contentType: "application/json; charset=utf-8",
              url: 'webIDNotesInfo.aspx?isAjax=true&autodel=1',
              dataType: "json",
              success: function (data) {
                if (!data.Success) { alert(data.ErrMsg); return; }
              },
...
<body id="body" runat="server">
    ...
    <form id="IDNotesInfo" name='IDNotesInfo' method="post" runat="server">
            ...
            <table class="table_maindiv">
                <tr>
                    <td>
                        <table class="table_common" cellspacing="0" cellpadding="0">
                            ...
                            <tr class="tr_common">
                                <td style="width: 120px" align="right" width="120" class="td_label" height="20">
                                    <asp:CheckBox ID="tx_validated_date" AutoPostBack="True" runat="server" Text="Final" Width="130" Font-Bold=true  OnCheckedChanged="Check_Clicked"/>                                
                                    <b></b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                </td>
                                <td width="5" height="20">
                                </td>
                                <td align="left" height="20">
                                    ...
                                    &nbsp;&nbsp;&nbsp;&nbsp;
                                    <asp:CheckBox ID="tx_verified_xix" runat="server" Text="Verify" Width="130" OnClick="DataChanged()" enabled="false" visible="true" />                                   
                                </td>
                ...
                <tr>
                    <td  style="height: 25px" align="center" class="td_label" height="25">
                        ...
                        <input type="button" value="Standard Text" style="width: 120px" id="selectabb"  runat="server" 
                             onclick="javascript:popupDialogSmallWide('../FormList/SearchAbbreviation.aspx');" />
                        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                        <input type="image" id="cm_notes_sr" alt="List" style="display:none;width: 25px;height:25px"  
                            src="../img/mic.png" title="Google SR" 
                            onclick="javascript:cmnotessr_click(); return false;" />
                    </td>
                </tr>
                <tr>
